...
| アクション | 責任者 | 期限 | 期待効果 |
|----------|-------|------|---------|
| journal に実施記録セクション追加 | self | IT-5 内（本ふりかえり時点で完了）✅ | ユーザー検証時の証跡 |
| アセット整合性 lint / GameTest を IT-6 計画タスクに追加 | self | IT-6 計画更新時 | 将来のアセット拡張時の安全網 |
| `.gen_textures.py` でドット模様パターン追加（識別性 +）| self | IT-6 着手前 | プレイヤーの視認性向上 |
| `release_plan.md` のベロシティ実績反映（5 IT 平均 8.8 SP）| self | 本ふりかえり時点 | 計画精度維持 |
