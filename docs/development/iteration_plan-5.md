...
| 1.4 | `runClient` クリエイティブインベントリで目視確認 | 0.5h | [x] ユーザー実施済み（2026-05-02）|
...
| # | タスク | 見積もり | 状態 |
|---|--------|---------|------|
| 2.1 | `runClient` で設置・破壊・回収のフローを実行 | 0.5h | [x] ユーザー実施済み（2026-05-02）|
...
| # | タスク | 見積もり | 状態 |
|---|--------|---------|------|
| 3.1 | `runClient` でクラフトテーブル体験を実行 | 0.5h | [x] ユーザー実施済み（2026-05-02）|
...
**進捗率**: 100%（7/7 SP）✅ ユーザーによる `runClient` 目視確認も完了（2026-05-02）

> **完了の経緯**: 初回 runClient で missing texture（紫×黒）が表示される問題が発生 → 1.21.x で必須化された `assets/aipe/items/*.json`（item definitions）の欠落と判明 → `commit 26f9fb4b` で追加して解消 → 再実行でテクスチャ・ブロック体験・クラフト体験すべて正常確認。
