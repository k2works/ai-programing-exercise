...
#### 1. US-001: Mod 起動確認（2 SP）✅ 完了

| # | タスク | 見積もり | 状態 |
|---|--------|---------|------|
| 1.1 | `apps/aipe` のパッケージ命名・MODID を最終確認（`aipe` で確定） | 0.5h | [x] |
| 1.2 | `runClient` を実行し起動ログを確認 | 1h | [x] |
| 1.3 | 起動確認手順を `docs/journal/it1-bootstrap.md` に記録 | 0.5h | [x] |

**小計**: 2h
**実績**: ユーザーがローカルで `./gradlew runClient` を実行し、Mod ID `aipe` のロード確認（2026-05-02）。
