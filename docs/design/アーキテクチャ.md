...
- Infrastructure In: Web/テンプレート/DTO変換
- Infrastructure Out: DBアクセス（JPA/Flyway）、外部システム
...
- アダプタ
  ...
  - Out: PersistenceAdapter（JPA + 悲観ロック）

契約の例（抜粋）
- ReservableRoomPort.findOneForUpdateByReservableRoomId(id): 悲観ロックで予約可能会議室を取得
- ReservationPort.findByReservableRoom_ReservableRoomIdOrderByStartTimeAsc(id): 開始時刻昇順取得
...
@startuml
actor User
User -> ReservationsController: POST /reservations/{date}/{roomId}
...
ReservableRoomPort --> ReservationService: ReservableRoom(lock=PESSIMISTIC_WRITE)
ReservationService -> ReservationPort: findByReservableRoom_..._OrderByStartTimeAsc(id)
...
- CSRF: 有効化、トークン付与
- UserDetails: AuthUserDetails（RoleName → ROLE_ 権限化）
- WebSecurityConfig: 静的リソース/ログインはpermitAll、その他authenticated
...
- 排他: @Lock(LockModeType.PESSIMISTIC_WRITE) を ReservableRoom 取得に適用
...
- E2E: 認証→会議室一覧→予約→キャンセル主要シナリオ
- 否定テスト: 権限外操作、重複予約、CSRF無効
