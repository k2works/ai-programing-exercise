...
- [x] 手順 6（クラフトテーブルを開く）が成功する
- [x] 手順 8（結果スロットに `example_item` が表示される）
- [x] 手順 9-10（取り出し + インベントリ確認）が成功する
...
| クラフトテーブル開く | ✅ OK（クリエイティブ検索タブで `crafting table` 検索 → 取得 → 設置 → 右クリック）|
| `example_item` 結果表示 | ✅ OK |
| 取り出し / インベントリ復帰 | ✅ OK |
| 備考 | クラフトテーブルはバニラアイテムのため検索タブで取得。GameTest（aipe:craft_block_to_item）が API レベルで保証しているレシピが GUI 経由でも成立 |
