...
- [x] 6.1 シードデータの作成
  - **ビジネス概要に基づくシードデータ仕様**:
    - **ユーザー**: スタッフ3名（管理者1名、一般スタッフ2名）、テスト用得意先5名
    - **仕入先**: 5社（各社に連絡先情報、取引状態を設定）
    - **単品**: 25種類（5社 × 平均5種類、品質維持可能日数3-7日、リードタイム1-3日）
    - **商品**: 10商品（主力商品、各商品5単品で構成、販売単価3000円前後、原価率70%）
    - **在庫**: 各単品に初期在庫を設定（ロット番号、入荷日、有効期限）
    - **得意先**: 10名（テストデータ用、連絡先情報を含む）
  - シードデータスクリプト（seed.ts）の実装
  - データの一貫性検証
  - _要件: 全体_
...
- [ ]* 26.1 在庫引当のプロパティベーステスト
  - **Property 11: 十分な在庫がある場合の引当成功**
  - **検証: 要件 7.3**

- [ ]* 26.2 在庫不足のプロパティベーステスト
...
- [ ]* 33.1 在庫ロット引当のプロパティベーステスト
  - **Property 13: 利用可能数量を超える引当は失敗**
  - **検証: 要件 11.2**

- [ ]* 33.2 有効期限切れ在庫のプロパティベーステスト
...
- [ ] 35. 在庫推移計算サービスの実装
  - InventoryProjectionServiceドメインサービス
  - 日次在庫予測（calculate）
  - 現在在庫の計算
  - 予定入荷の計算
  - 引当済数量の計算
...
- [ ]* 35.1 在庫推移計算のプロパティベーステスト
  - **Property 15: 在庫推移の一貫性**
  - **検証: 要件 12.1, 12.2, 12.3**

- [x] 36. 単品・仕入先管理サービスの実装
  - 単品登録（registerItem）
  ...
  - 仕入先無効化/再有効化
  - 参照整合性の維持
  - _要件: 10.1, 10.2, 10.3, 10.4, 10.5, 19.1, 19.2, 19.3, 19.4_

- [x] 37. 在庫管理APIエンドポイントの実装
  - POST /api/items
  - PUT /api/items/:id
  - GET /api/items/:id
  - POST /api/suppliers
  ...
  - _要件: 10.1, 10.2, 10.3, 10.5, 11.5, 12.1, 19.1, 19.2, 19.3, 19.4_
...
- [ ]* 38.1 発注リードタイムのプロパティベーステスト
  - **Property 16: 配送日はリードタイム日数後以降**
  - **検証: 要件 13.3**

- [ ]* 38.2 購入単位数量のプロパティベーステスト
...
- [ ] 53. データベーストランザクション管理の実装
  - トランザクション開始/コミット/ロールバック
  - 参照整合性の維持
  - データ制約のバリデーション
  - _要件: 20.3, 20.4, 20.5_

- [ ]* 53.1 トランザクションのプロパティベーステスト
  - **Property 22: トランザクション失敗時のロールバック**
  - **検証: 要件 20.4**

- [ ] 54. シリアライゼーション/デシリアライゼーションの実装
  - エンティティのシリアライズ
  - エンティティのデシリアライズ
  - 型安全性の確保
  - _要件: 20.1, 20.2_

- [ ]* 54.1 シリアライゼーションのプロパティベーステスト
...
- [x] 61. 発注管理UIの実装（スタッフ向け）
  - 発注作成フォーム
  - 発注一覧
  - 入荷・検収UI
  - _要件: 13.1, 13.2, 14.1, 14.2_

- [x] 62. 出荷管理UIの実装（スタッフ向け）
  - ピッキングリスト表示
  - 出荷実行UI
  - 返品処理UI
  - _要件: 15.1, 15.3, 16.1, 17.1_
...
- [ ] 63. プロパティベーステストライブラリのセットアップ
  - fast-checkライブラリのインストール
  - テストヘルパー関数の作成
  - ジェネレーター関数の基本実装
  - _要件: 全体_
