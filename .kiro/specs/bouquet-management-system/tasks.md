...
- [ ]* 25.1 在庫引当のプロパティベーステスト
  - **Property 11: 十分な在庫がある場合の引当成功**
  - **検証: 要件 7.3**

- [ ]* 25.2 在庫不足のプロパティベーステスト
...
- [ ]* 31.1 在庫ロット引当のプロパティベーステスト
  - **Property 13: 利用可能数量を超える引当は失敗**
  - **検証: 要件 11.2**

- [ ]* 31.2 有効期限切れ在庫のプロパティベーステスト
...
- [ ]* 36.1 発注リードタイムのプロパティベーステスト
  - **Property 16: 配送日はリードタイム日数後以降**
  - **検証: 要件 13.3**

- [ ]* 36.2 購入単位数量のプロパティベーステスト
...
- [ ]* 39.1 入荷検収のプロパティベーステスト
  - **Property 18: 受入時の在庫追加**
  - **検証: 要件 14.3**

- [ ] 40. 発注管理APIエンドポイントの実装
...
- [ ]* 44.1 返品期限のプロパティベーステスト
  - **Property 21: 30日より古い注文は返品不可**
  - **検証: 要件 17.4**

- [ ] 45. 出荷管理APIエンドポイントの実装
