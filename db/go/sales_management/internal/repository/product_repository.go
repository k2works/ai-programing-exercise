...
// 共通の SELECT カラムリスト（NULL 許容カラムに COALESCE を適用）
const productSelectColumns = `"商品コード", "商品正式名", "商品略称",
...
func (r *ProductRepository) FindByID(prodCode string) (*model.Product, error) {
	query := `SELECT ` + productSelectColumns + ` FROM "商品マスタ" WHERE "商品コード" = $1`
...
func (r *ProductRepository) FindAll() ([]*model.Product, error) {
	query := `SELECT ` + productSelectColumns + ` FROM "商品マスタ" ORDER BY "商品コード"`
...
// CreateWithContext は商品を登録します（Context 対応）
func (r *ProductRepository) CreateWithContext(ctx context.Context, exec Execer, product *model.Product) error {
	query := `
		INSERT INTO "商品マスタ" (
			"商品コード", "商品正式名", "商品略称", "商品名カナ", "商品区分", "製品型番",
			"販売単価", "仕入単価", "売上原価", "税区分",
...
func (r *ProductRepository) FindByIDWithContext(ctx context.Context, exec Execer, prodCode string) (*model.Product, error) {
	...
	var product model.Product
	err := exec.GetContext(ctx, &product, query, prodCode)
	if err != nil {
		if err == sql.ErrNoRows {
			return nil, nil
		}
...
func (r *ProductRepository) FindAllWithContext(ctx context.Context, exec Execer) ([]*model.Product, error) {
	...
	var products []*model.Product
	err := exec.SelectContext(ctx, &products, query)
	if err != nil {
		return nil, fmt.Errorf("商品一覧の取得に失敗しました: %w", err)
	}
...
// UpdateWithContext は商品を更新します（Context 対応）
func (r *ProductRepository) UpdateWithContext(ctx context.Context, exec Execer, product *model.Product) error {
	query := `
		UPDATE "商品マスタ"
		SET "商品正式名" = $1,
		    "商品略称" = $2,
...
// DeleteWithContext は商品を削除します（Context 対応）
func (r *ProductRepository) DeleteWithContext(ctx context.Context, exec Execer, prodCode string) error {
	query := `DELETE FROM "商品マスタ" WHERE "商品コード" = $1`

	result, err := exec.ExecContext(ctx, query, prodCode)
	if err != nil {
		return fmt.Errorf("商品の削除に失敗しました: %w", err)
	}
