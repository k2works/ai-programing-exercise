module SalesManagement.Tests.IntegrationTests.PurchaseOrderDetailTests
...
type PurchaseOrderDetailTests() =
    ...
    let createTestPoDetail poNo lineNo productCode productName quantity unitPrice =
        {
            PoNo = poNo
            PoLineNo = lineNo
            ProductCode = productCode
    ...
    member this.``発注明細を登録できる``() =
        task {
            ...
            let detail = createTestPoDetail "PO001" 1 "PROD001" "テスト商品" 100 500

            // Act
            do! PurchaseOrderDetailRepository.insertAsync this.ConnectionString detail

            // Assert
            let! details = PurchaseOrderDetailRepository.findByPoNoAsync this.ConnectionString "PO001"
            let detailList = details |> Seq.toList
            detailList |> should haveLength 1
            detailList.[0].ProductCode |> should equal "PROD001"
            detailList.[0].PoQuantity |> should equal 100
        }

    [<Fact>]
    member this.``発注明細の入荷済数量を更新できる``() =
        task {
            // Arrange
            do! setupCompany this.ConnectionString "COMP001"
            do! setupEmployee this.ConnectionString "EMP001"
            ...
            // Act
            do! PurchaseOrderDetailRepository.updateReceivedQuantityAsync this.ConnectionString "PO001" 1 80

            // Assert
            let! details = PurchaseOrderDetailRepository.findByPoNoAsync this.ConnectionString "PO001"
            let detailList = details |> Seq.toList
            detailList.[0].ReceivedQuantity |> should equal 80
        }

    [<Fact>]
    member this.``発注明細を削除できる``() =
        task {
            // Arrange
            do! setupCompany this.ConnectionString "COMP001"
            do! setupEmployee this.ConnectionString "EMP001"
            ...
            // Act
            do! PurchaseOrderDetailRepository.deleteByPoNoAsync this.ConnectionString "PO001"
    ...
    member this.``複数の発注明細を登録できる``() =
        task {
            ...
            let detail1 = createTestPoDetail "PO001" 1 "PROD001" "テスト商品" 100 500
            let detail2 = createTestPoDetail "PO001" 2 "PROD001" "テスト商品" 50 500

            do! PurchaseOrderDetailRepository.insertAsync this.ConnectionString detail1
            do! PurchaseOrderDetailRepository.insertAsync this.ConnectionString detail2
            ...
            detailList |> List.sumBy (fun d -> d.PoQuantity) |> should equal 150
        }
