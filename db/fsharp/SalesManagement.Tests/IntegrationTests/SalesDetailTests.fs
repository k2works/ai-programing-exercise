module SalesManagement.Tests.IntegrationTests.SalesDetailTests
...
type SalesDetailTests() =
    ...
    let setupSales (connectionString: string) salesSlipNo customerCode customerBranch employeeCode deptCode =
        task {
            let sales = {
    ...
    let createTestDetail salesSlipNo lineNo productCode productName quantity unitPrice =
        {
            SalesSlipNo = salesSlipNo
            SalesLineNo = lineNo
            ProductCode = productCode
    ...
    member this.``売上明細を登録できる``() =
        task {
            ...
            do! setupSales this.ConnectionString "SAL001" "COMP001" 1 "EMP001" "10000"
            do! setupProduct this.ConnectionString "PROD001"

            let detail = createTestDetail "SAL001" 1 "PROD001" "商品A" 10 1000

            // Act
            do! SalesDetailRepository.insertAsync this.ConnectionString detail

            // Assert
            let! details = SalesDetailRepository.findBySalesSlipNoAsync this.ConnectionString "SAL001"
    ...
    member this.``複数の明細を登録できる``() =
        task {
            ...
            let detail1 = createTestDetail "SAL001" 1 "PROD001" "商品A" 10 1000
            let detail2 = createTestDetail "SAL001" 2 "PROD002" "商品B" 5 2000

            do! SalesDetailRepository.insertAsync this.ConnectionString detail1
            do! SalesDetailRepository.insertAsync this.ConnectionString detail2
    ...
    [<Fact>]
    member this.``売上を削除すると明細もカスケード削除される``() =
        task {
            // Arrange
            do! setupCompany this.ConnectionString "COMP001"
            do! setupEmployee this.ConnectionString "EMP001"
    ...
    member this.``商品名と単価は履歴として保持される``() =
        task {
            ...
            // 売上明細に商品マスタとは異なる名前と単価を設定
            let detail = createTestDetail "SAL001" 1 "PROD001" "旧商品名" 10 900

            do! SalesDetailRepository.insertAsync this.ConnectionString detail

            // Act - 商品マスタを更新
            let! product = ProductRepository.findByIdAsync this.ConnectionString "PROD001"
            let updatedProduct = { product.Value with ProductFormalName = "新商品名"; SellingPrice = 1100 }
            do! ProductRepository.updateAsync this.ConnectionString updatedProduct

            // Assert - 売上明細は旧データを保持
            let! details = SalesDetailRepository.findBySalesSlipNoAsync this.ConnectionString "SAL001"
            let detailList = details |> Seq.toList
            detailList.[0].ProductName |> should equal "旧商品名"
            detailList.[0].UnitPrice |> should equal 900
        }
