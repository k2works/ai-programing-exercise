...
import infrastructure.entity.AlternateProduct
...
trait AlternateProductRepository {
  ...
  def findById(prodCode: String, altProdCode: String)(implicit
    session: DBSession
  ): Option[AlternateProduct]
...
class AlternateProductRepositoryImpl extends AlternateProductRepository {
  ...
  override def findById(prodCode: String, altProdCode: String)(implicit
    session: DBSession
  ): Option[AlternateProduct] =
    sql"""
  ...
  override def findByProduct(prodCode: String)(implicit
    session: DBSession
