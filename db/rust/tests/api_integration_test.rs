...
/// テスト用アプリケーションセットアップ
async fn setup_test_app() -> (Router, PgPool, &'static Container<'static, Postgres>) {
    // Dockerクライアントとコンテナを'staticライフタイムでリーク
    let docker = Box::leak(Box::new(Cli::default()));
    let container: &'static Container<'static, Postgres> =
        Box::leak(Box::new(docker.run(Postgres::default())));
