 ...
 * パフォーマンス最適化: React.memo、useMemo、useCallback使用
 */
export const GamePage: React.FC<GamePageProps> = React.memo(
  ({ gameService, inputHandler, gameRenderer, className = '' }) => {
    // ゲーム状態管理フック
    const {
      gameState,
      isPlaying,
    ...
    // スコアハイライトの判定（メモ化）
    const isScoreHighlighted = useMemo(
      () => scoreHighlight || Date.now() - lastScoreUpdate < 2000,
      [scoreHighlight, lastScoreUpdate]
    );
    ...
    const scoreDisplayProps = useMemo(
      () => ({
      ...
      [gameState?.score, isScoreHighlighted, isAnimating]
    );

    const nextPuyoDisplayProps = useMemo(
      () => ({
        nextPuyoPair: gameState?.nextPuyoPair,
        visible: showNextPuyo,
        className: `${styles['next-puyo-section']} next-puyo-section`,
      }),
      [gameState?.nextPuyoPair, showNextPuyo]
    );
      ...
      >
        ...
        >
          ...
          <aside className={`${styles['left-panel']} left-panel`}>
            {showScore && gameState && scoreDisplayProps.score && (
          ...
          {/* ゲームフィールド */}
          <section
            className={`${styles['game-field-section']} game-field-section`}
          >
            {gameState && gameBoardProps.field && (
              <>
          ...
          <aside className={`${styles['right-panel']} right-panel`}>
            {showNextPuyo && gameState && nextPuyoDisplayProps.nextPuyoPair && (
              <NextPuyoDisplay {...nextPuyoDisplayProps} />
            )}
        ...
        {gameState && gameState.isGameOver && (
          ...
          >
            <div className={`${styles['game-over-dialog']} game-over-dialog`}>
              ...
              <div
                className={`${styles['game-over-buttons']} game-over-buttons`}
              >
                <button
                  type="button"
                  onClick={handleNewGame}
        ...
        <div className="sr-only" aria-live="polite">
          {gameState &&
            gameState.isGameOver &&
            `ゲームオーバー。最終スコア: ${gameState.score.current}`}
          {gameState &&
            gameState.chainCount > 0 &&
            `${gameState.chainCount}連鎖発生`}
        </div>
      </main>
    );
  }
