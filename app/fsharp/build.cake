...
Setup(ctx =>
{
    Information("=== F# 財務会計システム データベース設計プロジェクト ビルド開始 ===");
...
Task("Restore")
    ...
    .Does(() =>
{
    DotNetRestore("./AccountingSystem.sln");
    ...
    .Does(() =>
{
    var exitCode = StartProcess("dotnet", new ProcessSettings {
        Arguments = "fsharplint lint AccountingSystem.sln"
    });

    if (exitCode != 0)
    {
        Warning("FSharpLintで警告が検出されました");
    }
    else
    {
...
Task("Build")
    ...
    .Does(() =>
{
    DotNetBuild("./AccountingSystem.sln", new DotNetBuildSettings
    {
...
Task("Test")
    ...
    .Does(() =>
{
    DotNetTest("./AccountingSystem.sln", new DotNetTestSettings
    {
        Configuration = configuration,
        NoBuild = true,
    ...
    .Does(() =>
{
    ...
    {
        WorkingDirectory = "./AccountingSystem.Infrastructure",
        Configuration = configuration
    };

    DotNetRun("./AccountingSystem.Infrastructure/AccountingSystem.Infrastructure.fsproj", settings);
