...
/// <summary>
/// 仕訳イベントソーシングサービス（イベント発行機能付き）
/// イベント保存後にイベントを発行して Read Model を更新
/// </summary>
type JournalEntryEventSourcingServiceWithEvents
    ...
    let saveAndPublishEventsAsync
        ...
        task {
            ...
            | Ok () ->
                // イベントを発行（Read Model 更新など）
                do! eventPublisher.PublishManyAsync events
                return Ok ()
        }
    ...
    interface IJournalEntryEventSourcingUseCase with
        ...
        member _.CreateJournalEntryAsync
            ...
            task {
                ...
                | Error msg -> return Error msg
                | Ok aggregate ->
                    let! saveResult = saveAndPublishEventsAsync id aggregate.UncommittedEvents 0 userId
