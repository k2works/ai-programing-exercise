...
/// <summary>
/// 仕訳イベントソーシング REST API コントローラー（スナップショット最適化版）（Input Adapter）
/// </summary>
[<ApiController>]
[<Route("api/v1/journal-entries-with-snapshot")>]
[<Tags("仕訳イベントソーシングAPI（スナップショット最適化版）")>]
type JournalEntryEventSourcingWithSnapshotController(journalEntryService: JournalEntryEventSourcingServiceWithSnapshot) =
