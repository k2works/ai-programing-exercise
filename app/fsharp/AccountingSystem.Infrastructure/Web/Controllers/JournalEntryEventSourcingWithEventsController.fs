...
/// <summary>
/// 仕訳イベントソーシング REST API コントローラー（イベント発行版）（Input Adapter）
/// イベント保存後に Read Model 更新と監査ログ記録を自動実行
/// </summary>
[<ApiController>]
[<Route("api/v1/journal-entries-with-events")>]
[<Tags("仕訳イベントソーシングAPI（イベント発行版）")>]
type JournalEntryEventSourcingWithEventsController(journalEntryService: JournalEntryEventSourcingServiceWithEvents) =
    inherit ControllerBase()

    let useCase = journalEntryService :> IJournalEntryEventSourcingUseCase
    ...
    member this.GetJournalEntry(id: string) : Task<IActionResult> =
        task {
            let! aggregate = useCase.GetJournalEntryAsync(id)
    ...
    member this.GetAllJournalEntries() : Task<IActionResult> =
        task {
            let! aggregates = useCase.GetAllJournalEntriesAsync()
    ...
    member this.GetJournalEntryAt(id: string, [<FromQuery>] pointInTime: DateTime) : Task<IActionResult> =
        task {
            let! aggregate = useCase.GetJournalEntryAtAsync id pointInTime
    ...
    /// <summary>
    /// 新しい仕訳を作成
    /// イベント保存後、Read Model 更新と監査ログ記録が自動実行される
    /// </summary>
    [<HttpPost>]
    ...
    member this.CreateJournalEntry([<FromBody>] request: CreateJournalEntryRequest) : Task<IActionResult> =
        task {
            ...
            let! result =
                useCase.CreateJournalEntryAsync
                    request.Id
                    request.EntryDate
                    request.Description
                    lineItems
    ...
    member this.ApproveJournalEntry(id: string, [<FromBody>] request: ApproveJournalEntryRequest) : Task<IActionResult> =
        task {
            ...
            let! result =
                useCase.ApproveJournalEntryAsync
                    id
                    request.ApprovedBy
                    comment
    ...
    member this.DeleteJournalEntry(id: string, [<FromBody>] request: DeleteJournalEntryRequest) : Task<IActionResult> =
        task {
            ...
            let! result =
                useCase.DeleteJournalEntryAsync
                    id
                    request.Reason
                    userId
    ...
    member this.CreateSnapshot(id: string) : Task<IActionResult> =
        task {
            let! result = journalEntryService.CreateSnapshotAsync(id)

            match result with
            | Ok () ->
                return this.Ok({ Success = true; Message = "スナップショットを作成しました" }) :> IActionResult
            | Error msg when msg.Contains("見つかりません") ->
                return this.NotFound(ErrorResponse.create 404 "NotFound" msg) :> IActionResult
            | Error msg ->
                return this.StatusCode(500, ErrorResponse.create 500 "InternalServerError" msg) :> IActionResult
        }
    ...
    [<ProducesResponseType(StatusCodes.Status204NoContent)>]
    member this.DeleteSnapshot(id: string) : Task<IActionResult> =
        task {
            do! journalEntryService.DeleteSnapshotAsync(id)
            return this.NoContent() :> IActionResult
        }
