...
public class JournalEntryEventSourcingApiTest : ApiTestBase
{
    ...
    [Fact(DisplayName = "POST /api/v1/journal-entries-es - 仕訳を作成できる")]
    ...
    [Fact(DisplayName = "POST /api/v1/journal-entries-es - 貸借不一致で400を返す")]
    ...
    [Fact(DisplayName = "GET /api/v1/journal-entries-es/{id} - 仕訳を取得できる")]
    public async Task GetJournalEntry_Returns200()
    {
        ...
        journal.Description.Should().Be("テスト仕訳");
        journal.Status.Should().Be("DRAFT");
        journal.Deleted.Should().BeFalse();
        journal.LineItems.Should().HaveCount(2);
    }

    [Fact(DisplayName = "GET /api/v1/journal-entries-es/{id} - 存在しないIDで404を返す")]
    ...
    [Fact(DisplayName = "POST /api/v1/journal-entries-es/{id}/approve - 仕訳を承認できる")]
    ...
    [Fact(DisplayName = "POST /api/v1/journal-entries-es/{id}/approve - 存在しないIDで404を返す")]
    ...
    [Fact(DisplayName = "POST /api/v1/journal-entries-es/{id}/approve - 既に承認済みで400を返す")]
    ...
    [Fact(DisplayName = "DELETE /api/v1/journal-entries-es/{id} - 仕訳を削除できる")]
    ...
    [Fact(DisplayName = "DELETE /api/v1/journal-entries-es/{id} - 存在しないIDで404を返す")]
    ...
    [Fact(DisplayName = "GET /api/v1/journal-entries-es/{id}/at - 特定時点の仕訳状態を取得できる")]
    ...
    [Fact(DisplayName = "GET /api/v1/journal-entries-es/{id}/at - 指定時点に存在しない場合404を返す")]
    ...
    [Fact(DisplayName = "複合操作: 作成 → 承認 → 削除")]
    public async Task ComplexOperations_CreateApproveDelet()
    {
        // Arrange & Act
        // 1. 作成
        var createRequest = CreateValidRequest();
