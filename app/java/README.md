...
開発環境やテスト環境にサンプルデータを投入するには、以下のコマンドを実行します：

```bash
# Seed データ投入（デフォルト: dev プロファイル）
./gradlew seedDatabase

# 特定のプロファイルで実行
./gradlew seedDatabase -PseedProfile=test
```

**投入されるデータ:**
- 勘定科目マスタ（資産、負債、純資産、収益、費用）
- 仕訳データ（令和3年度、令和4年度）
- 日次勘定科目残高（令和3年度、令和4年度）

**注意事項:**
- Seed データ投入は `accounting.seed.enabled=true` が設定された環境でのみ実行されます
- 既存のデータはクリアされませんので、必要に応じて事前に `flywayClean` を実行してください
