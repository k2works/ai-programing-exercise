...
func Test_新アーキテクチャ_FizzBuzzValueCommand_タイプ1_3を渡したら文字列Fizzを返す(t *testing.T) {
	command := application.NewFizzBuzzValueCommand(types.CreateFizzBuzzType(1))
	expected := "Fizz"
	actual := command.Execute(3)
	if actual != expected {
...
func Test_新アーキテクチャ_FizzBuzzListCommand_100までの配列を返す(t *testing.T) {
...
// モジュール分割後のValue Objectのテスト
func Test_新アーキテクチャ_FizzBuzzValue_値オブジェクトの生成(t *testing.T) {
...
// モジュール分割後のファーストクラスコレクションのテスト
func Test_新アーキテクチャ_FizzBuzzList_新しいインスタンスが作られる(t *testing.T) {
	// 初期リスト作成
